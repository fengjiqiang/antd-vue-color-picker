...
const handleDrag = event => {
  ...
  if (!props.vertical) {
    ...
    hue = Math.round(
      ((left - (thumb.value as HTMLElement).offsetWidth / 2) / (rect.width - (thumb.value as HTMLElement).offsetWidth)) * 360
    )
  } else {
    ...
    hue = Math.round(
      ((top - (thumb.value as HTMLElement).offsetHeight / 2) / (rect.height - (thumb.value as HTMLElement).offsetHeight)) * 360
    )
  }
