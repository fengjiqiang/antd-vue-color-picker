...
import { ref, getCurrentInstance, shallowRef, watch, onMounted } from 'vue'
...
const handleDrag = event => {
  ...
  if (!props.vertical) {
    ...
    props.color?.set(
      'alpha',
      Math.round(
        ((left - (thumb.value as HTMLElement).offsetWidth / 2) / (rect.width - (thumb.value as HTMLElement).offsetWidth)) * 100
      )
    )
  } else {
    ...
    props.color?.set(
      'alpha',
      Math.round(
        ((top - (thumb.value as HTMLElement).offsetHeight / 2) / (rect.height - (thumb.value as HTMLElement).offsetHeight)) * 100
      )
    )
  }
}
